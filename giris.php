	...
	if (empty($e_posta_err) && empty($parola_err)) {
		...
		if ($result->num_rows == 1) {
			...
			if (password_verify($password, $hashli_sifre)) {
				...
				$_SESSION["adres"] = $kullanici["adres"];
...
<head>
	...
	position: relative;
	width: 380px;
	height: 480px; /* Yüksekliği artırdık */
	background: #1c1c1c;
	border-radius: 8px;
	overflow: hidden;
}
		...
		.guest-link {
	position: relative; /* Form kapsayıcısına göre konumlandır */
	z-index: 3; /* Üst katmanda görünmesini sağla */
	display: block; /* Blok öğe olarak hizala */
	margin: 20px auto; /* Yukarıdan ve aşağıdan boşluk bırak, ortala */
	font-size: 1em;
	text-align: center; /* Yazıyı ortala */
	color: #45f3ff;
	text-decoration: none;
	background: #1c1c1c; /* Arka plan rengi */
	padding: 10px 20px; /* İç boşlukları artır */
...
.guest-link:hover {
	color: #fff;
	background: #45f3ff; /* Hover sırasında arka plan rengi değişsin */
}
...
<body>
	...
	<div class="box">
		...
		<!-- Misafir olarak devam et bağlantısı -->
		 
	</div>
	<a href="anasayfa.php" class="guest-link">Misafir olarak devam et</a>
